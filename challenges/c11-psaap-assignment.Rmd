...
-   mu_f and d_p have some of the smallest values compared to the other variables.

-   I_0, N_p, and avg_q have some of the largest values compared to the other variables.
...
-   Which model is *most accurate*? Which is *least accurate*?
    -   The cheating model was most accurate and the nonphysical model was least accurate.
-   What *Category* of variable is `avg_T`? Why is it such an effective predictor?
    -   avg_T is in the Output category, meaning it's a result of the same test that we obtain the T_norm value from.
-   Would we have access to `avg_T` if we were trying to predict a *new* value of `T_norm`? Is `avg_T` a valid predictor?
    -   No, we would not have access to avg_T, so it cannot be a valid predictor.
-   What *Category* of variable is `idx`? Does it have any physical meaning?
    -   idx is in the Metadata category, so it doesn't actually affect the test and is instead just used to keep track of the value.
...
-   Which model tends to be more accurate? How can you tell from this predicted-vs-actual plot?
    -   The q4 model seems slightly more accurate, as the trend line better fits the data points.
-   Which model tends to be *more confident* in its predictions? Put differently, which model has *narrower prediction intervals*?
    -   The q4 model seems to be more confident in its predictions, given its narrower prediction intervals.
...
-   How much do you trust your model? Why?
    -   The model doesn't have a lot of data, so it might not be the best model to predict an interval.
-   What kind of interval---confidence or prediction---would you use for this task, and why?
    -   A prediction interval would be used, since I want to make sure that the distribution of outputs fall within the predicted output.
...
-   What interval for `T_norm` would you recommend the design team to plan around?
    -   Based on the results above, I would not want to recommend an interval for T_norm due to the lack of conclusive data.
-   Are there any other recommendations you would provide?
    -   I would recommend managing the input better, since it is something that greatly affects the accuracy of the prediction, and is also something that is controllable by humans when conducting the test.
