...
- Write your observations here!
  - While the mean velocities of all of the different images were similar (with a MeanVelocity difference of around 50 km/s), the majority of the images had a Distinctness of 3.
  
- Why might your table differ from Michelson's?
  - The table might differ due to rounding, so implementing ```round()``` would account for those value differences.
...
- Is Michelson's estimate of the error (his uncertainty) greater or less than the true error?
  - Michelson's estimate of the error was less than the true error.
- Make a quantitative comparison between Michelson's uncertainty and his error.
  - Michelson's uncertainty was plus or minus 51 km/s, but the true error was 151.142 km/s, which falls outside of his uncertainty, which is around 3 times Michalson's uncertainty.
...
**Observations**:
Similarities
- The general behavior and trend of the velocity data seems consistent, where they generally seem to be increasing and decreasing along the same time intervals, which must be due to chance since the simulated data is purely random. Plus, they both are centered around the same range.

Differences
- There appears to be greater variance in the real data, which has more outliers, such as with the data before June 09. 
...
Accoring to the first plot, there doesn't seem to be any correlation between Temp and VelocityVacuum. Also, a Distinctness of 3 appears generally more towards the higher temperatures after 70 degrees. Based on this, we it seems as though the temperature does not affect the difference between Michelson's estimate and `LIGHTSPEED_VACUUM`.
According to the second plot, the median VelocityVacuum value increased as the Distinctness increased, but this is a weak correlation since they are relatively close to each other. In addition, the Distinctness of 1 has one outlier that is less than the rest of the VelocityVacuum data, Distinctness of 2 has two outliers (one greater than and one less than the rest of the data), and Distinctness of 3 doesn't seem to have any outliers. This does not give us much insght into the difference between Michelson's estimate and `LIGHTSPEED_VACUUM`.
