...
- mu_f and d_p have some of the smallest values compared to the other
  variables.

- I_0, N_p, and avg_q have some of the largest values compared to the
  other variables.
...
df_psaap %>% 
  ...
  geom_boxplot() +
  facet_grid(~x) +
  ggtitle("T_norm at Different Channel Locations (x)") 
```
...
- Which model is *most accurate*? Which is *least accurate*?
  - The cheating model was most accurate and the nonphysical model was
    least accurate.
- What *Category* of variable is `avg_T`? Why is it such an effective
  predictor?
  - avg_T is in the Output category, meaning it’s a result of the same
    test that we obtain the T_norm value from.
- Would we have access to `avg_T` if we were trying to predict a *new*
  value of `T_norm`? Is `avg_T` a valid predictor?
  - No, we would not have access to avg_T, so it cannot be a valid
    predictor.
- What *Category* of variable is `idx`? Does it have any physical
  meaning?
  - idx is in the Metadata category, so it doesn’t actually affect the
    test and is instead just used to keep track of the value.
...
T_f and x seem to be the most useful variables.
...
- Which model tends to be more accurate? How can you tell from this
  ...
  - The q4 model seems slightly more accurate, as the trend line better
...
- Which model tends to be *more confident* in its predictions? Put
  ...
  - The q4 model seems to be more confident in its predictions, given
    its narrower prediction intervals.
- How many predictors does the `fit_simple` model need in order to make
  a prediction? What about your model `fit_q4`?
  - fit_simple needs 1 predictor, and fit_q4 needs 2 predictors.
...
df_validate <- df_validate %>% 
  arrange(desc(T_norm)) %>% 
  mutate(
    in_range = (df_intervals$pi_lwr < T_norm) & (T_norm < df_intervals$pi_upr)
  ) %>% 
  summarize(
    total = n(),
    num_in_range = sum(in_range),
    perc_in_range = num_in_range / total
  )
...
```
    ...
    ## # A tibble: 1 × 3
    ##   total num_in_range perc_in_range
    ##   <int>        <int>         <dbl>
    ## 1    60            3          0.05
...
- How much do you trust your model? Why?
  - The model doesn’t have a lot of data, so it might not be the best
    model to predict an interval.
- What kind of interval—confidence or prediction—would you use for this
  task, and why?
  - A prediction interval would be used, since I want to make sure that
    the distribution of outputs fall within the predicted output.
- What fraction of validation cases lie within the interval you predict?
  How does this compare with `pr_level`?
  - 3 of 60 cases lie within the interval predicted.
- What interval for `T_norm` would you recommend the design team to plan
  around?
  - Based on the results above, I would not want to recommend an
    interval for T_norm due to the lack of conclusive data.
- Are there any other recommendations you would provide?
  - I would recommend managing the input better, since it is something
    that greatly affects the accuracy of the prediction, and is also
    something that is controllable by humans when conducting the test.
