...
### __q2__ The following code attempts to visualize `cty, hwy` from `mpg` using `geom_col()`. There's something fishy about the `hwy` values; what's wrong here?
...
**Observations**:
Since position = "stacked" is the default for geom_col(), we don't see the real hwy values, but effectively a sum at each cty value.
...
**Observations**:
SUV’s are most numerous, followed by compact and midsize. There are very few 2seater vehicles.
...
**Observations**:
SUV’s are most numerous, followed by compact and midsize. There are very few 2seater vehicles. However, now we additionally know that pickup’s and SUV’s tend to have 4 wheel drive, compact’s and midsize tend to have f drive, and all the 2seater vehicles are r drive.
...
v1 showed the same variable (class) using two aesthetics, while v2 showed two variables (class and drv) using two aesthetics
...
**Observations**
Certain manufacturers seem to favor particular classes of car. For instance, it appears that Jeep, Land Rover, Lincoln, and Mercury only have SUV’s, Audi, Toyota, and Volkswagen favor compact, and Dodge favors pickup.

### __q5__ The following graph is hard to read. What other form of faceting would make the visual more convenient to read? Modify the code below with your suggested improvement.
